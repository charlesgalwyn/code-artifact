...
from core.model_gru import GRUModel
...
def main():
    ...
    save_dir = os.path.join(model_cfg.get('save_dir', 'saved_models'), 'gru')
    ...
    print("[Run-GRU] Vocab sizes:", vocab_sizes)
    print("[Run-GRU] Numeric features:", num_info.get('numeric_cols', []))
    ...
    print("[Run-GRU] Train windows:", X_action_train.shape, X_num_train.shape, "Test windows:", X_action_test.shape, X_num_test.shape)
    ...
    gru.build_model(configs, vocab_sizes=vocab_sizes, n_numeric=n_numeric, timesteps=timesteps)
    ...
    print("[Run-GRU] Class weights:", class_weight_dict)
    ...
    # save history
    hist_out = os.path.join(save_dir, "training_history_gru.csv")
    pd.DataFrame(history.history).to_csv(hist_out, index=False)
    print("[Run-GRU] Saved training history to:", hist_out)

    # evaluate
    y_pred = gru.predict_classes(X_test_list)
    ...
    cm_path = os.path.join(save_dir, "confusion_matrix_gru.png")
    plot_confusion(cm, loader.get_label_mapping(), cm_path)
    print("[Run-GRU] Saved confusion matrix to:", cm_path)

    # save predictions
    out_csv = os.path.join(save_dir, "predictions_vs_true_gru.csv")
    pd.DataFrame({"y_true": y_test, "y_pred": y_pred}).to_csv(out_csv, index=False)
    print("[Run-GRU] Saved predictions CSV to:", out_csv)

    print("[Run-GRU] Completed. Artifacts saved in:", os.path.abspath(save_dir))
